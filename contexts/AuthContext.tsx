  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session?.user) {
        ...
        // Vérifier d'abord app_metadata, puis user_metadata en fallback
    ...
    const {
      ...
      if (session?.user) {
        ...
        console.log('🔄 Auth state changed - App metadata:', session.user.app_metadata);
        
        // Vérifier d'abord app_metadata, puis user_metadata en fallback
        const role = session.user.app_metadata?.role || session.user.user_metadata?.role || 'client';
        console.log('👤 Rôle après changement:', role);
        setIsAdmin(role === 'admin');
      } else {
        setIsAdmin(false);
      }
